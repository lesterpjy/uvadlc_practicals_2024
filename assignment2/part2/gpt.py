...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.view(B, T, 3, self.n_head, C // self.n_head).permute(
            2, 0, 3, 1, 4
        )  # (3, B, nh, T, hs)
        q = qkv[0]
        k = qkv[1]
        ...
        if self.use_flash_attn:
            ...
            att = torch.einsum("bhie,bhje->bhij", q, k)  # (B, nh, T, T)
            att = att * (1.0 / math.sqrt(k.size(-1)))  # scale by sqrt d_k
            # Apply causal mask
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_out = self.self_attention(x_norm_1)
